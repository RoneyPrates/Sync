        ...
        if (Array.isArray(produtos)) {
            ...
            if (produtosAtivos.length > 0) {
                produtosAtivos.forEach(produto => {
                    // Verifica se o produto já foi adicionado à ordem
                    const produtoNaOrdem = produtosSelecionados.find(p => p.produto_id === produto.id);
                    const quantidade = produtoNaOrdem ? produtoNaOrdem.quantidade : 1; // Se o produto já está na ordem, usa a quantidade atual
